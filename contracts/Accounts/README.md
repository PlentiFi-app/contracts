...
```bash
forge script script/preview_deployed_addresses.s.sol --rpc-url <your_rpc_url> --private-key <your_private_key>
```

5. Deploy the Implementation Manager:
```bash
forge script script/deploy/deployImplementationManagerDeterministic.s.sol --broadcast  --rpc-url <your_rpc_url>  --private-key <your_private_key>
```

6. Deploy the Factory Staker:
```bash
# Note: This doesn't need to be deterministic
forge script script/deploy/deployFactoryStakerDeterministic.s.sol --broadcast --rpc-url <your_rpc_url>  --private-key <your_private_key>
```

7. Deploy your Account Factory:
```bash
forge script script/deploy/deployAccountFactoryDeterministic.s.sol --broadcast --rpc-url <your_rpc_url>  --private-key <your_private_key>
```
...
```bash
forge script script/deploy/deployProxyUpgrader.s.sol --broadcast --rpc-url <your_rpc_url>  --private-key <your_private_key>
```
